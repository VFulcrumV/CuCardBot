...
class Moderators(commands.Cog):
    ...
    @commands.slash_command(name="accrual_to_the_user_balance")
    @commands.has_permissions(administrator=True)
    async def award(self, ctx, user: disnake.Member = None, amount: int = None):
        if int(amount) < 1:
            await ctx.response.send_message(
                f"Минимальная сумма выдаваемых мемокоинов: **1** :coin:",
                ephemeral=True
            )
        else:
    ...
    @commands.slash_command(name="charges_from_the_user_balance")
    @commands.has_permissions(administrator=True)
    async def take_away_coins(self, ctx, user: disnake.Member = None, amount=None):
        if int(amount) < 1:
            await ctx.response.send_message(
                f"Минимальная сумма забираемых мемокоинов: **1** :coin:",
                ephemeral=True
            )
        else:
